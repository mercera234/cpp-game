...
SpiralBlitAnimation::SpiralBlitAnimation(Image* background, bool playForwards)
{
	...
	origY = getScreenHeight() / 2;
	origX = getScreenWidth() / 2;
...
void SpiralBlitAnimation::takeStep()
{
	...
	else
	{
		...
		while (qualityStep == false && tilesChangedCount < getScreenHeight() * getScreenWidth())
		{
			...
			if (curY >= 0 && curX >= 0 && curY < getScreenHeight() && curX < getScreenWidth()) //only blit if still on screen
			{
				changeTile(curY, curX);
				qualityStep = true;
			}
			...
			if (curY >= 0 && curX + 1 >= 0 && curY < getScreenHeight() && curX + 1 < getScreenWidth()) //only blit if still on screen
			{
